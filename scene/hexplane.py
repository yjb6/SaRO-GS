...
def grid_sample_wrapper(grid: torch.Tensor, coords: torch.Tensor,levels,spatio_only,max_level=None, align_corners: bool = True) -> torch.Tensor:
    ...
    levels=torch.min(levels,dim=-1).values.unsqueeze(0).unsqueeze(0)#use the min level to get high accuracy

    #scale-aware intrep, Eq 18 in the main paper
    interp =nvdiffrast.torch.texture(
            ...
            boundary_mode="clamp",
            max_mip_level=7 if spatio_only else 0,
        )  # 3xNx1xC
...
def interpolate_ms_features(pts: torch.Tensor,
    ...
    for scale_id,  grid in enumerate(ms_grids[:num_levels]):
        ...
        for ci, coo_comb in enumerate(coo_combs):
            ...
            spatio_only = (3 not in coo_comb)
            feature_dim = grid[ci].shape[1]  # shape of grid[ci]: 1, out_dim, *reso
            interp_out_plane = (
                grid_sample_wrapper(grid[ci], pts[..., coo_comb],levels[...,coo_comb],spatio_only)
                .view(-1, feature_dim)
            )
...
class ScaleAwareResField(nn.Module):
    def __init__(
        self,
    ...
    def set_aabb(self,xyz_max, xyz_min,duration):
        ...
        max_level = torch.log(torch.tensor(self.reso_list[0]))
        base_scale = torch.tensor(self.per_grid_size[0],dtype=torch.float32,device="cuda")

        self.register_buffer("max_level",max_level)
        self.register_buffer("base_scale",base_scale)
        # self.register_buffer("reso_list", self.reso_list)
    ...
    def get_level(self,scales:torch.Tensor):
        ...
        level = torch.cat((level,torch.zeros((level.shape[0],1)).to(level)),dim=-1)
        level[:,3] = 0.0
