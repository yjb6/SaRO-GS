...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  image_name=cam_info.image_name, uid=id, data_device=args.data_device, near=cam_info.near, far=cam_info.far, timestamp=cam_info.timestamp, rayo=rays_o, rayd=rays_d,meta_only=args.use_loader)



def loadCamv2(args, id, cam_info, resolution_scale):
    #resolution只会改变图像的大小，FoV在读取camera信息时已经确定
    orig_w, orig_h = cam_info.image.size
    # print(orig_w,orig_h)
    if args.resolution in [1, 2, 4, 8]:
        resolution = round(orig_w/(resolution_scale * args.resolution)), round(orig_h/(resolution_scale * args.resolution))
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY, 
                  image=gt_image, gt_alpha_mask=loaded_mask,
                  image_name=cam_info.image_name, uid=id, data_device=args.data_device, near=cam_info.near, far=cam_info.far, timestamp=cam_info.timestamp, rayo=rays_o, rayd=rays_d,cxr=cam_info.cxr,cyr=cam_info.cyr,mask=mask,meta_only=args.use_loader,resolution=resolution,image_path=cam_info.image_path)
                  #加上了mask
